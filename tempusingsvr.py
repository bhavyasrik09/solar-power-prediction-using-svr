...
svr1.fit(x_train, y_temp_train.ravel())
svr2.fit(x_train, y_ghi_train.ravel())
...
try:
    for _ in range(10):  # Run for 10 iterations (or replace with while True for infinite loop)
        ...
        temp = svr1.predict(now)[0]  # Predict temperature
        ghi = svr2.predict(now)[0]   # Predict GHI
        ...
        sql = """INSERT INTO power_predictionsvr (time_updated, Temperature, GHI, power)
                 VALUES (%s, %s, %s, %s)"""
